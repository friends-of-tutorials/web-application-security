...
Headereinstellungen können je nach verwendetem Webserver mittels seiner Webservereinstellungen, via Konfigurationsdateien z.B. der `.htaccess`-Datei (NCSA kompatible Webserver<sup>1</sup>) oder direkt in der Webanwendung selbst gesetzt werden (z.B. mittels einer Scriptsprache wie PHP, Ruby, Python, etc.). 
